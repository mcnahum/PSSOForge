...
function Main {
    ...
    # Build output filename: "macOS _ PSSO <Name> (<SE|PSync>)_<timestamp>.json"
    $timestamp = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH_mm_ss.fffZ')
    $safeDisplayName = $policyName -replace '[<>:"/\\|?*]', '_'
    $fileName = "${safeDisplayName}_${timestamp}.json"
